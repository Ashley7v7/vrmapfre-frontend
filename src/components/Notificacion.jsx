...
export default function Notificacion({ notificaciones }) {
  ...
  const nombreUsuario = normalizar(localStorage.getItem('nombreCompleto') || '');
  ...
  const filtrarNotificaciones = (lista) => {
    ...
    return lista
      .filter((n) => {
        if (normalizar(n.suscriptor) !== nombreUsuario) return false;

        if (n.estatus === 'Agendada') return true;

        if (['Cancelada', 'Visitada'].includes(n.estatus)) {
          let fechaCruda = null;

          if (n.estatus === 'Cancelada') {
            fechaCruda = n.fechaCancelacion || n.fechaSolicitud || n.fecha;
          } else if (n.estatus === 'Visitada') {
            fechaCruda = n.fechaVisitada; // ✅ SOLO esta cuenta para visitadas
          }
                ...
                >
                  ...
                  <p className="text-sm text-gray-700">
                    ...
                    {new Date(
                      n.estatus === 'Visitada'
                        ? n.fechaVisitada
                        : n.fechaVisita || n.fechaSolicitud || n.fecha
                    ).toLocaleDateString()}
